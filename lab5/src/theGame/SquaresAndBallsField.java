...
public class SquaresAndBallsField implements BallsField {
    ...
    private UserInterface userInterface = new UserInterface() {
        ...
        @Override
        public void nextMoment(double passedSeconds) throws FailException, WinException { }
        @Override
        public void update() {}
        @Override
    ...
    @Override
    public void setUI(UserInterface userInterface) throws NullPointerException, IOException {
        if (userInterface == null)
            throw new NullPointerException();
        this.userInterface = userInterface;
        userInterface.drawRectangle(0, 0, 1, 1, UserInterface.RectangleTypes.EMPTY);
    ...
    @Override
    public BallsField recreate() {
        SquaresAndBallsField res = new SquaresAndBallsField(cols, topEmptyRows, filledRows, bottomEmptyRows);
        for (int i = 0; i < res.points.length; ++i)
            res.points[i] = points[i];
        for (Ball cur : balls) {
            res.balls.add(new Ball(cur.x, cur.y, cur.vx, cur.vy, cur.radius * 2));
            ++res.numberOfBalls;
        }
        return res;
    }
